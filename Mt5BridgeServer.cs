...
class Program {
    ...
    // ── Connect ───────────────────────────────────────────────────────────────
    // Real signature from DLL scan:
    // MTRetCode Connect(String server, UInt64 login, String password,
    //                   String password_cert, EnPumpModes pump_mode, UInt32 timeout)
    static void Connect() {
        lock(_lock) {
            try {
                ...
                rc = _mgr.Connect(
                    ...
                    MGR_LOGIN,
                    MGR_PASSWORD,
                    "",                                              // password_cert (empty = none)
                    CIMTManagerAPI.EnPumpModes.PUMP_MODE_USERS |
                    CIMTManagerAPI.EnPumpModes.PUMP_MODE_DEALS |
                ...
                _connected = true;
                Console.WriteLine("[MGR] ✅ Connected to " + MGR_SERVER + " as login " + MGR_LOGIN);
            } catch(Exception ex){
    ...
    // ── MT5 operations — all using real signatures from DLL scan ──────────────

    // UserAccountGet(UInt64 login, CIMTAccount obj) — REAL method name
    // UserGet(UInt64 login, CIMTUser obj) — REAL method name
    ...
    // PositionGet(UInt64 login, CIMTPositionArray positions) — REAL method name
    ...
    // DealRequest(UInt64 login, Int64 from, Int64 to, CIMTDealArray deals) — REAL method name
        ...
        lock(_lock){ if(!_connected||_mgr==null)return"[]";
            try{
                ...
                for(uint i=0;i<arr.Total();i++){
                    ...
                    sb.Append("{\"ticket\":"+d.Deal()+",\"symbol\":\""+Esc(d.Symbol())+"\",\"type\":\""+t+"\",\"lots\":"+((double)d.Volume()/10000.0).ToString("F2")+",\"open_price\":"+d.Price().ToString("F5")+",\"profit\":"+d.Profit().ToString("F2")+",\"open_time\":\""+DateTimeOffset.FromUnixTimeSeconds(d.Time()).DateTime.ToString("yyyy-MM-dd HH:mm:ss")+"\",\"comment\":\""+Esc(d.Comment())+"\",\"account\":"+login+"}");
                }
                sb.Append("]");arr.Dispose();return sb.ToString();
            }catch(Exception ex){Console.WriteLine("[History] "+ex.Message);return"[]";}}
    }

    // DealerBalance(UInt64 login, Double value, UInt32 type, String comment, UInt64& deal_id)
    // — used for close-all via balance adjustment NOT available via DealerDeal on this interface
    // Instead use DealerSend(CIMTRequest request, CIMTDealerSink sink, UInt32& id)
            ...
            try{
                ...
                for(uint i=0;i<total;i++){
                    ...
                    try{
                        ...
                        // DealerSend(CIMTRequest request, CIMTDealerSink sink, UInt32& id)
                        // sink can be null for fire-and-forget
                        uint reqId=0;
                        MTRetCode ret=_mgr.DealerSend(req,null,ref reqId);
    ...
    // UserAdd(CIMTUser user, String master_pass, String investor_pass) — REAL signature
    // DealerBalance(UInt64 login, Double value, UInt32 type, String comment, UInt64& deal_id) — REAL
    static string CreateAccount(string name,string group,double balance,int leverage) {
        lock(_lock){ if(!_connected||_mgr==null)return"{\"error\":\"Manager not connected\"}";
            try{
                ...
                // Disable trade right from the start (Point 6: locked until battle starts)
                ulong rights=user.Rights();
                rights &= ~(ulong)CIMTUser.EnUsersRights.USER_RIGHT_TRADE;
                user.Rights(rights);

                string pw=MakePw(); string inv=MakePw();
                MTRetCode ret=_mgr.UserAdd(user,pw,inv);
                if(ret!=MTRetCode.MT_RET_OK){user.Dispose();return"{\"error\":\"UserAdd failed code="+ret+". Check group name: '"+Esc(group)+"' in MT5 Manager Groups tab.\"}";}

                ulong nl=user.Login();user.Dispose();

                // Deposit starting balance
                // DealerBalance(login, value, type=0=balance, comment, out deal_id)
                ulong dealId=0;
                _mgr.DealerBalance(nl,balance,0,"MFT Battle Starting $"+balance.ToString("F0"),ref dealId);

                Console.WriteLine("[Create] login="+nl+" name="+name+" balance="+balance+" TRADING LOCKED");
    ...
    // UserUpdate(CIMTUser obj) — REAL method name
    // Rights flags: USER_RIGHT_TRADE enables/disables trading
    ...
    static bool Deposit(ulong login,double amount,string comment) {
        lock(_lock){ if(!_connected||_mgr==null)return false;
            try{
                ulong dealId=0;
                MTRetCode ret=_mgr.DealerBalance(login,amount,0,comment,ref dealId);
                return ret==MTRetCode.MT_RET_OK;
            }catch{return false;}}
    }
}
